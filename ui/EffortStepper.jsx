...
export function EffortStepper({ provider, value, onChange, disabled = false }) {
  ...
  return (
    <div className={`nw-effort${disabled ? ' is-disabled' : ''}`}>
      <div
        className="nw-effort-track"
        role="radiogroup"
        ...
        {levels.map((level, idx) => {
          ...
          return (
            <button
              ...
              type="button"
              className={`nw-effort-stop${idx <= selectedIndex ? ' is-filled' : ''}${on ? ' is-active' : ''}`}
              role="radio"
              aria-checked={on}
      ...
      <span className="nw-effort-label">{selected?.label || levels[0].label}</span>
    </div>
  )
}
